...
const tcpSocketServer = net.createServer((socket) => {
  ...
  // 연결 종료된 드론 삭제
  const disconnectedDroneId = socket.remoteAddress; // 드론 ID를 소켓 정보로 추적할 수 있다면 변경 필요
  if (drones[disconnectedDroneId]) {
    delete drones[disconnectedDroneId]; // drones에서 드론 삭제
    console.log(`[드론 삭제됨]: ${disconnectedDroneId}`);

    // WebSocket을 통해 삭제된 드론 정보 전송
    broadcastToWebSocketClients({
      type: "drone-deleted",
      data: disconnectedDroneId,
    });
  }
